...
class MacTest(SetupInput):
	...
	@patch('mac.get_input', return_value=[])
	def test_no_imput(self, input):
		self.assertEqual(mac.parse_play(), ['Error: no argument for XML tree'])
	...
	def test_tree_arg(self):
		self.assertEqual(mac.parse_play(self.play)[0], 'Macbeth has 719')
		self.assertEqual(mac.parse_play(self.play)[1], 'Lady Macbeth has 265')
